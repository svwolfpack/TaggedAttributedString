...
+ (TASParseResult *)parseOpeningCommand:(NSString *)command argument:(NSString *)argument openTag:(TASTag *)openTag
{
    ...
    if ([command isEqualToString:kTASForegroundColorKey1] || [command isEqualToString:kTASForegroundColorKey2])
    {
        tag = [[TASForegroundColorTag alloc] init];
    }
    else if ([command isEqualToString:kTASBackgroundColorKey])
    {
        tag = [[TASBackgroundColorTag alloc] init];
    }
    else
    {
        return nil;
